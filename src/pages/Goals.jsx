...
const emptyForm = () => ({
  name: '', category: 'other', target_amount: '', current_saved: '0',
...
export default function Goals() {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    const record = { ...form, target_amount: parseFloat(form.target_amount) || 0, current_saved: parseFloat(form.current_saved) || 0, monthly_contribution: parseFloat(form.monthly_contribution) || 0, is_active: form.is_active };
    if (editItem) {
      await base44.entities.Goal.update(editItem.id, record);
    } else {
  ...
  const totalSaved = activeGoals.reduce((s, g) => s + (g.current_saved || 0), 0);
  ...
  return (
    <div className="px-4 pt-14 pb-6 space-y-4 max-w-lg mx-auto">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-extrabold">{lang === 'zh' ? '目标规划' : 'Goals'}</h1>
        <Button onClick={openAdd} size="sm" className="h-9 rounded-xl font-semibold bg-primary">
          <Plus className="w-4 h-4 mr-1" />{lang === 'zh' ? '新目标' : 'New Goal'}
        </Button>
      </div>
      ...
      {/* Goal cards */}
      {goals.length === 0 && (
        <p className="text-center text-muted-foreground text-sm py-8">{lang === 'zh' ? '尚未设置目标' : 'No goals yet. Start planning!'}</p>
      )}
      <div className="space-y-3">
        {goals.map(g => <GoalCard key={g.id} goal={g} lang={lang} onEdit={() => openEdit(g)} />)}
      </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }}
              ...
              <FormField label={lang === 'zh' ? '目标名称 *' : 'Goal Name *'}>
                <input value={form.name} onChange={e => set('name', e.target.value)} placeholder={lang === 'zh' ? '例如：旅行基金' : 'e.g. Travel Fund'} className="w-full bg-secondary rounded-xl px-3 py-2.5 text-sm font-medium focus:outline-none focus:ring-1 focus:ring-primary" />
              </FormField>
              ...
              <div className="grid grid-cols-2 gap-3">
                <FormField label={lang === 'zh' ? '目标金额 (RM) *' : 'Target Amount (RM) *'}>
                ...
                <FormField label={lang === 'zh' ? '已储蓄 (RM)' : 'Current Saved (RM)'}>
              ...
              <div className="grid grid-cols-2 gap-3">
                <FormField label={lang === 'zh' ? '月存金额 (RM)' : 'Monthly Contribution'}>
...
function GoalCard({ goal, lang, onEdit }) {
  ...
  const dailyExtra = monthsNeeded ? (remaining / (monthsNeeded * 20)).toFixed(2) : null;
    ...
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
      ...
      {(monthsNeeded || requiredMonthly) && (
        <div className="bg-purple-50 rounded-xl p-3 text-xs text-purple-700">
          ...
          {dailyExtra && <p>🚗 {lang === 'zh' ? `每工作日多赚 RM${dailyExtra}` : `Earn extra RM${dailyExtra}/working day`}</p>}
          {requiredMonthly && targetDate && <p>🎯 {lang === 'zh' ? `达到目标需每月存 RM${requiredMonthly}` : `Need RM${requiredMonthly}/mo to hit target date`}</p>}
        </div>
      )}
    </motion.div>
  );
